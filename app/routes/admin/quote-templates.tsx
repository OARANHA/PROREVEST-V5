...
import { PDFService, type QuoteTemplate } from "../../services/pdfService";

export const meta: MetaFunction = () => {
  return [
    { title: "Templates de Orçamento - ProRevest" },
    { name: "description", content: "Gerencie os templates de orçamento da ProRevest" },
  ];
}
...
export default function AdminQuoteTemplates() {
  ...
  const [filteredTemplates, setFilteredTemplates] = useState<QuoteTemplate[]>([]);

  // Carregar templates de orçamento
  useEffect(() => {
    const loadTemplates = async () => {
      try {
        const templatesData = await PDFService.getQuoteTemplates();
        setTemplates(templatesData);
        setFilteredTemplates(templatesData);
      } catch (error) {
        console.error("Erro ao carregar templates de orçamento:", error);
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleDeleteTemplate = async (templateId: string) => {
    try {
      await PDFService.deleteQuoteTemplate(templateId);
  ...
  const handleDownloadTemplate = (template: QuoteTemplate) => {
    // Em uma implementação real, isso faria o download do template
    const blob = new Blob([template.content], { type: 'application/json' });
    ...
    <div className="p-6">
      <div className="mb-6">
        <div className="flex justify-between items-center">
          <div>
            <h1 className="text-3xl font-cormorant font-bold">Templates de Orçamento</h1>
            <p className="text-muted-foreground">Gerencie os templates de orçamento da ProRevest</p>
          </div>
